...
async def get_dynamic_from_user(uid: int):
    usr_ins = user.User(uid)
    ...
    if page["cards"][0]["card"]["item"]["orig_dy_id"]:
        ...
        print("short_link:"+str(orig_d_info["card"]["short_link"]))

    return page["cards"][0]["card"]["item"]["content"]
""""""
# 动态点赞 给某条动态点赞(需要输入credential)
...
if __name__ == "__main__":
    ...
    sync(get_dynamic_from_user(uid_list[0]))
